...
import com.admin.coredge.SpeedCalculation.DownloadSpeed.DownloadStream;
...
import com.admin.coredge.SpeedCalculation.UploadSpeed.UploadStream;
...
public abstract class SpeedtestWorker extends Thread {
    private ServerTestPoint backend;
    private SpeedConfig config;
    private TelConfig telemetryConfig;
    private boolean stopASAP=false;
    private double dl=-1, ul=-1, ping=-1, jitter=-1;
    private String ipIsp="";
    private Logger log=new Logger();

    public SpeedtestWorker(ServerTestPoint backend, SpeedConfig config, TelConfig telemetryConfig){
        this.backend=backend;
        this.config=config==null?new SpeedConfig():config;
        this.telemetryConfig=telemetryConfig==null?new TelConfig():telemetryConfig;
        start();
    }
    ...
    private void getIP(){
        ...
        try {
            ...
            if (config.getErrorHandlingMode().equals(SpeedConfig.ONERROR_FAIL)){
                abort();
                onCriticalFailure(t.toString());
            }
            return;
    ...
    private void sendTelemetry(){
        if(telemetryConfig.getTelemetryLevel().equals(TelConfig.LEVEL_DISABLED)) return;
        if(stopASAP&&telemetryConfig.getTelemetryLevel().equals(TelConfig.LEVEL_BASIC)) return;
        try{
